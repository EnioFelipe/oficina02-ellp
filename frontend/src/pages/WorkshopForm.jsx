...
export default function WorkshopForm() {
  ...
  useEffect(() => {
    usersService.list().then((users) => {
      setResponsibles(users.filter((user) => ['professor', 'tutor'].includes(user.type)));
      if (!id) setLoading(false);
    });
    if (!id) return;
    workshopsService.get(id).then((workshop) => {
      setForm({
        name: workshop.name,
  ...
  return (
    <form className="form stack" onSubmit={handleSubmit}>
      ...
      <label>
        Responsável
        <select required value={form.professor} onChange={(e) => setForm({ ...form, professor: e.target.value })}>
          <option value="">Selecione</option>
          {responsibles.map((user) => (
            <option key={user._id} value={user._id}>{user.name} ({user.type})</option>
          ))}
        </select>
      </label>
      <label>
